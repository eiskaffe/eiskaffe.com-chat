...
def main():
    ...
    cipher = b64decode(requests.request("GET", f"{URL}/pub?username="+state["username"], headers={}, data={}).text)
    with open(f"users/{state['username']}.privatekey", "rb") as prv:
        PRIVATE_KEY = PKCS1_OAEP.new(RSA.importKey(prv.read()))
        
    with open(f"users/{state['username']}.authkey", "r", encoding="utf-8") as f:
        next(f)
        key = ""
        for line in f:
    ...
    serverSocket.send(f"{state['username']} {b64encode(cipher).decode()}".encode()) # authenticating client
    ...
    serverSocket.send(aes.encrypt(state["groupname"]))
    response = serverSocket.recv(1024).decode("utf-8")
